...
extern bt_done;
extern void intimate_bt(uint8_t *);
...
//      return;
//#endif
#if (GATT_CL_GAR_BV_04_C || GATT_CL_GAR_BI_13_C || GATT_CL_GAR_BV_07_C )
      ...
      rsi_ble_app_set_event(RSI_BLE_GATT_READ);
#elif (GATT_CL_GAR_BV_07_C)
      rsi_ble_app_set_event (RSI_BLE_GATT_DESC_SERVICES);
#endif
      return;
#endif
#endif
    } else if((resp_status >= 0x4a80) && (resp_status <= 0x4aff)) {
      LOG_PRINT ("\n Att error Response - Appilication error \n");
    ...
    memcpy(&ble_read_val, rsi_ble_resp_att_val, sizeof(rsi_ble_resp_att_value_t));
#if (GATT_CL_GAR_BV_01_C || GATT_CL_GAR_BV_03_C || GATT_CL_GAR_BV_04_C || GATT_CL_GAR_BV_05_C || GATT_CL_GAR_BV_06_C || GATT_CL_GAR_BV_07_C || GATT_CL_GPA_BV_02_C || GATT_CL_GPA_BV_03_C || GATT_CL_GPA_BV_04_C || GATT_CL_GPA_BV_05_C || GATT_CL_GPA_BV_06_C || GATT_CL_GPA_BV_07_C || GATT_CL_GPA_BV_08_C || GATT_CL_GAD_BV_05_C)

    LOG_PRINT ("\n ble_read_val length : %d",ble_read_val.len);
    LOG_PRINT ("\n ble_read_val data : %s \n",&ble_read_val.att_value[0]);

//#if (GATT_CL_GAR_BV_07_C)
//    rsi_ble_app_set_event (RSI_BLE_GATT_DESC_SERVICES);
//    return;
//#endif
#if (GATT_CL_GAR_BV_04_C || GATT_CL_GAR_BV_07_C)

    if(ble_read_val.len < (ble_mtu.mtu_size -1))
    {
      counter_flag++;
      dbg_flag =0;
      if(counter_flag >= 4)
    ...
    case RSI_BLE_CONN_EVENT:
      {
        ...
        intimate_bt(app_conn_event.dev_addr);
#endif
#if (GATT_CL_GAC_BV_01_C || GATT_CL_GAW_BV_05_C || GATT_CL_GAW_BI_07_C || GATT_CL_GAW_BI_08_C || GATT_CL_GAW_BI_09_C \
...
reconnect:
        ...
        intimate_bt(remote_dev_bd_addr);
#endif
...
//          rsi_6byte_dev_address_to_ascii ((int8_t *)remote_dev_addr, app_conn_event.dev_addr);
          ...
          if (!dbg_flag) {
            LOG_PRINT("Enter Hndl val: ");
            LOG_SCAN("%x", &first_handle);
#if (GATT_CL_GAR_BV_04_C || GATT_CL_GAR_BI_12_C || GATT_CL_GAR_BI_14_C || GATT_CL_GAR_BI_17_C)
            length = 0;
#else
            LOG_PRINT("Enter offset in decimal: ");
            LOG_SCAN("%d", &length);
