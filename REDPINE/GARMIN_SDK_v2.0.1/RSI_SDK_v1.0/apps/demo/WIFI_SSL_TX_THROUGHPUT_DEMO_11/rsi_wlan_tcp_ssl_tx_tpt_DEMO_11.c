/**
 * @file    rsi_wlan_tcp_ssl_tx_tpt_DEMO_11.c
 * @version 0.1
 * @date    13 Nov 2018
 *
...
/*====================================================*/
/**
 * @fn         void  rsi_app_task_wifi_ssl_tx_tpt(void)
...
#endif
	{
		...
		case RSI_WLAN_IPCONFIG_DONE_STATE:
		{
			...
			else
			{
				...
				LOG_PRINT("TCP/SSL TX test started...\n");
			}
		}
		case RSI_WLAN_SOCKET_CONNECTED_STATE:
		{
			...
			{
#if 0
				for (pkt_length = 1; pkt_length <= BUF_SIZE; pkt_length++)
#else
					pkt_length = BUF_SIZE;
#endif
				{
					//! Send data on socket
					status = rsi_send(client_socket,send_buf, pkt_length, 0);
					if(status < 0)
					{
						status = rsi_wlan_get_status();
						LOG_PRINT("Send failed, closing socket\n");
						rsi_shutdown(client_socket, 0);
						return;
					}
					num_bytes += pkt_length;
					pkt_count++;

					if(pkt_count == MAX_TX_PKTS)
					{
						while(!data_recvd)
						{
