...
static uint8_t   remote_dev_addr[6];
...
int8_t createFile_sd()
{
	...
	PRINTF("\r\nOpening a file on SDCard\r\n");

	//! Create a file on SD card
	error = f_open(&g_fileObject_n, _T(INPUT_FILE_NAME), ( FA_READ ));
	if (error)
	{
		if (error == FR_EXIST)
		{
...
static uint32_t rsi_ble_add_simple_chat_serv1(void) {
	...
	rsi_ble_add_char_serv_att(new_serv_resp.serv_handler,
			...
			RSI_BLE_ATT_PROPERTY_READ | RSI_BLE_ATT_PROPERTY_WRITE | RSI_BLE_ATT_PROPERTY_NOTIFY,
...
{
  memcpy ((int8_t *)remote_dev_addr, resp_enh_conn->dev_addr, 6);
...
{
	memcpy ((int8_t *)remote_dev_addr, resp_conn->dev_addr, 6);
	LOG_PRINT("\nconnect - str_remote_address : %s\r\n",
...
static void rsi_ble_on_read_req_event(uint16_t event_id,
	...
	memcpy ((int8_t *) remote_dev_addr,
			rsi_ble_read_req->dev_addr, 6);
	LOG_PRINT("\nOffset value is %d\n", offset);
...
void rsi_ble_on_smp_request(rsi_bt_event_smp_req_t *remote_dev_address) {
	memcpy ((int8_t *)remote_dev_addr, remote_dev_address->dev_addr, 6);
...
void rsi_ble_on_smp_passkey_display(
	...
	memcpy ((int8_t *)remote_dev_addr, smp_passkey_display->dev_addr, 6);
...
void rsi_ble_on_sc_passkey(rsi_bt_event_sc_passkey_t *sc_passkey) {
	memcpy ((int8_t *)remote_dev_addr, sc_passkey->dev_addr, 6);
...
int32_t rsi_ble_peripheral_role(void) {
	...
	rsi_ble_gatt_register_callbacks(ble_on_profiles_list_resp, rsi_ble_profile,
...
int32_t rsi_ble_app_task() {
	...
	case RSI_BLE_READ_REQ_EVENT: {
		...
		if (req_type == 1) {
			rsi_ble_gatt_read_response((uint8_t *) remote_dev_addr, 1,
					app_ble_read_event.handle, offset,
					(sizeof(read_data1) - offset), &(read_data1[offset]));
			offset = 0;
		} else {

			rsi_ble_gatt_read_response((uint8_t *) remote_dev_addr, 0,
	...
	case RSI_BLE_DISCONN_EVENT: {
		LOG_PRINT("\n Device disconnected\n ");
		rsi_ble_app_clear_event(RSI_FILE_READ_EVENT);
#if ENABLE_POWER_SAVE
		//! initiating power save in BLE mode
	...
	case RSI_BLE_RECEIVE_REMOTE_FEATURES: {
		...
		if (remote_dev_feature.remote_features[0] & 0x20) {
			status = rsi_ble_set_data_len(remote_dev_addr, TX_LEN, TX_TIME);
			...
			status = rsi_ble_setphy((int8_t *)remote_dev_addr, TX_PHY_RATE,
			...
			status = rsi_ble_smp_pair_request(remote_dev_addr,
	...
	case RSI_BLE_SMP_REQ_EVENT: {
		...
		if (first_connect == 0) {
			...
			status = rsi_ble_start_encryption(remote_dev_addr,
	...
	case RSI_BLE_SMP_RESP_EVENT: {
		...
		//! initiating the SMP pairing process
		status = rsi_ble_smp_pair_response(remote_dev_addr,
				RSI_BLE_SMP_IO_CAPABILITY, MITM_ENABLE);
	}
	break;
	...
	case RSI_BLE_SMP_PASSKEY_EVENT: {
		...
		//! initiating the SMP pairing process
		status = rsi_ble_smp_passkey(remote_dev_addr, RSI_BLE_APP_SMP_PASSKEY);
	}
	break;
	...
	case RSI_BLE_LTK_REQ_EVENT: {
		...
		//rsi_6byte_dev_address_to_ascii((int8_t *)remote_dev_addr,	temp_le_ltk_req.dev_addr);
	...
	case RSI_BLE_ENCRYPT_STARTED_EVENT: {
		...
		status = rsi_ble_conn_params_update((int8_t *)remote_dev_addr, 24, 36, 0, 400);
		if (status == RSI_SUCCESS) {
			rsi_conn_update = 1;
		}
	}
	...
	case RSI_BLE_CONN_UPDATE_EVENT: {
		...
		if (rsi_conn_update == 1) {
			...
			retry1: status = rsi_ble_get_profile_async(remote_dev_addr,
			...
			get_prof: status = rsi_ble_get_profiles_async(remote_dev_addr,
		...
		if (rsi_conn_update == 2) {
			rsi_ble_disconnect((int8_t *)remote_dev_addr);
			rsi_conn_update = 0;
	...
	case RSI_BLE_GATT_WRITE_EVENT:
	{
		...
		rsi_ble_app_set_event(RSI_FILE_READ_EVENT);
	}
	break;
	case RSI_BLE_PROFILE_LIST_EVENT:
	{
		...
		if (!profiles_list_done) {
			...
			status = rsi_ble_get_profiles_async(remote_dev_addr,
...
case RSI_FILE_READ_EVENT:
  {
    uint16_t bytesRead;
    memset(data, 0, sizeof(data));

    error = f_read(&g_fileObject_n, data, sizeof(data), &bytesRead);
    if (error) {
      PRINTF("\n Read file failed. \r\n");
      return error;
    }
	...
	rsi_ble_set_local_att_value(rsi_ble_att2_val_hndl,
			bytesRead, (uint8_t *) data);
    }
    //LOG_PRINT("\n notification sent for %d length data\n",bytesRead);

#if SYNC_COMMUNICATION_ENABLED
	rsi_ble_app_clear_event(RSI_FILE_READ_EVENT);
#endif
