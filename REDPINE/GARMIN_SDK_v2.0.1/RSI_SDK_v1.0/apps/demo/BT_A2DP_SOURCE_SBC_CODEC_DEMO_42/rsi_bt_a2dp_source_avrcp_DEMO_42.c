/**
 * @file    rsi_bt_a2dp_source_avrcp_DEMO_42.c
 * @version 0.1
 * @date    13 Oct 2018
 *
...
/* Read buffer */
static uint8_t  g_bufferRead[512*20];
#endif
...
extern int32_t rsi_bt_a2dp_get_config(uint8_t *remote_dev_addr, rsi_bt_resp_a2dp_get_config_t *sbc_resp_cap);
extern int32_t rsi_bt_a2dp_set_config(uint8_t *remote_dev_addr, rsi_bt_a2dp_sbc_codec_cap_t *set_sbc_cap, uint32_t *status);
...
void rsi_bt_app_on_a2dp_connect(uint16_t resp_status,
  ...
  LOG_PRINT ("on a2dp conn: bd addr %s, resp status 0x%x\r\n",
      rsi_6byte_dev_address_to_ascii((int8_t *)str_conn_bd_addr, a2dp_connect->dev_addr), resp_status);
}
...
int16_t send_audio_data(FILE *fp) {
	...
	err = rsi_bt_a2dp_send_pcm_mp3_data (remote_dev_addr, g_bufferRead, 512*20, RSI_AUDIO_DATA_TYPE);
#elif (RSI_AUDIO_DATA_TYPE == MP3_AUDIO)
 ...
 */
int32_t rsi_app_task_bt_a2dp_source_sbc_codec() {
	int32_t status = 0;
