/**
 * @file    rsi_bt_spp_master_sniff_DEMO_34.c
 * @version 0.1
 * @date    xx Feb 2019
 *
 ...
 *
 *  @brief : This file contains example application for BT Classic SPP Master Role for sniff
 *
 *  @section Description  This application serves as a BT SPP Master
 *
 */
...
int32_t rsi_app_task_bt_spp_master_sniff(void)
{
	...
	while(1)
	{
		...
		case RSI_APP_EVENT_AUTH_COMPLT:
		{
			...
			status = rsi_bt_spp_connect((uint8_t *)remote_dev_addr);
			if(status != RSI_SUCCESS)
			{
				LOG_PRINT ("status =%x \n",status);
			}
