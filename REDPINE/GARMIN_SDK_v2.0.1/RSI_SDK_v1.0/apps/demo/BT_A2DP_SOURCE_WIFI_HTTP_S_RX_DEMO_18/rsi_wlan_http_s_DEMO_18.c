/**
 * @file    rsi_wlan_http_s_DEMO_18.c
 * @version 0.1
 * @date    13 Nov 2018
 *
...
#endif
{
	...
	case RSI_WLAN_CONNECTED_STATE:
	{
		...
		LOG_PRINT("Loaded SSL CA Certificate\n");
#endif
	...
	{
		if(stop_download || data_processed)
			break;

		//! Check whether HTTP download complete
		status = rsi_app_wlan_data_recvd();
		if(status != RSI_SUCCESS)
		{
#ifdef RSI_WITH_OS
			continue;
	...
	case RSI_WLAN_SOCKET_CONNECTED_STATE:
	{
        ...
        //! Changing:HP Chain in EDR MODE when WLAN is connected 
		rsi_bt_ptt_req(EDR_MODE);
#endif
	...
	case RSI_WLAN_DATA_RECEIVE_STATE:
	{
		...
		{
#endif
			ret = rsi_app_process_http_data();
			if(ret < 0){
				LOG_PRINT("HTTP processing failed\n");
#ifdef RSI_WITH_OS
			continue;
#else
			return;
...
#if BT_BR_EDR_ADAPTIVE        
        //! Changing:LP Chain in BR MODE when WLAN is IDLE 
		rsi_bt_ptt_req(BR_MODE);
#endif
