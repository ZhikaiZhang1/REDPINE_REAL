/**
 * @file    rsi_bt_a2dp_source_avrcp_DEMO_20.c
 * @version 0.1
 * @date    xx Apr 2018
 *
 ...
 *
 *  @section Description contains example application for BT Classic A2DP Source and AVRCP target
 *
 */
...
volatile uint8_t  audio_played = 0;
static uint32_t   rsi_app_async_event_map = 0;
static uint32_t   rsi_app_async_mask_map = 0;
...
#if (RSI_AUDIO_DATA_TYPE == SBC_AUDIO)
extern uint8_t total_data_received;
extern uint8_t http_resp_ok;
...
//   ! PROCEDURES
/*=======================================================================*/
void rsi_bt_app_mask_event(uint32_t event_num);
void rsi_bt_app_unmask_event(uint32_t event_num);
void rsi_bt_app_set_event(uint32_t event_num);
void rsi_bt_app_clear_event(uint32_t event_num);
...
/*==============================================*/
/**
 * @fn         rsi_bt_app_mask_event
 * @brief      Masks the event parameter.
 * @param[in]  none.
 * @return     none.
 * @section description
 * This function is used to mask the specific event.
 */
void rsi_bt_app_mask_event(uint32_t event_num) {
	rsi_app_async_mask_map |= BIT(event_num);
	return;
}
 ...
 */
void rsi_bt_app_set_event(uint32_t event_num) {
	if(!(rsi_app_async_mask_map & BIT(event_num)))
	{
#ifdef RSI_WITH_OS
...
void rsi_bt_on_confirm_request(uint16_t resp_status,
	...
	LOG_PRINT("\n on confirm req: bd addr %s, resp status 0x%x\r\n",
			rsi_6byte_dev_address_to_ascii(str_conn_bd_addr, remote_dev_addr), resp_status);
}
...
void rsi_bt_app_a2dp_pause() {
	...
	if (status != RSI_SUCCESS) {
		LOG_PRINT("\n A2DP suspend req: bd addr %s, status err 0x%x\r\n",
				rsi_6byte_dev_address_to_ascii(str_conn_bd_addr,remote_dev_addr), status);
	} else {
		app_state &= ~(1 << A2DP_STREAM_START);
...
//				break;
			...
			do {
				...
				if(!audio_played)
				{
					...
					status = RSI_APP_ERR_AUDIO_EOF;

				if (status == RSI_APP_ERR_AUDIO_EOF) {
#if SD_DEMO
					if (app_song_list_idx == (SONG_PLAY_COUNT - 1))
					{
						audio_played = 1;
						if(total_data_received && http_resp_ok)
						{
							rsi_bt_app_clear_event(RSI_APP_EVENT_A2DP_MORE_DATA_REQ);
							rsi_bt_app_mask_event(RSI_APP_EVENT_A2DP_MORE_DATA_REQ);

							rsi_wlan_app_cb.state = RSI_SD_WRITE_STATE;
							LOG_PRINT("SD WRITE Started...\n");
						}
						else
							return;
					}
