/**
 * @file    rsi_ble_dual_role_DEMO_3.c
 * @version 0.1
 * @date    09 Sep 2015
 *
...
static void rsi_ble_on_connect_event(rsi_ble_event_conn_status_t *resp_conn)
{
  ...
  memcpy (remote_dev_bd_addr, (uint8_t *) resp_conn->dev_addr, 6);
	LOG_PRINT("\n connected device address : %s",str_conn_device);
...
int32_t rsi_ble_dual_role(void) {
	...
	//! Set local name
	rsi_bt_set_local_name((int8_t *)RSI_BLE_APP_GATT_TEST);
...
void rsi_ble_app_task(void) {
		...
		if (conn_req_pending == 0) {
			if (num_of_conn_slaves < RSI_BLE_MAX_NBR_SLAVES) {
				...
				if (((strcmp((const char *) remote_name,
					...
					if (status != RSI_SUCCESS) {
						LOG_PRINT("\n Scan stopping failed with status : %d \n",
								status);
						//return status;
					}
					rsi_app_no_of_adv_reports_rcvd = 0;
					...
					if (status != RSI_SUCCESS) {
						LOG_PRINT("\n Connecting failed with status : %d \n", status);
						//return status;
						rsi_ble_app_set_event(RSI_BLE_SCAN_RESTART_EVENT);
						break;
					}
	...
	case RSI_BLE_CONN_EVENT: {
		...
		//status = rsi_ble_get_profile(conn_event_to_app[conn_done-1].dev_addr, service_uuid, &rsi_ble_service);
	...
	case RSI_BLE_GATT_WRITE_EVENT: {
		...
		rsi_ble_set_local_att_value(0x000C, RSI_BLE_MAX_DATA_LEN, (uint8_t *) data);
	...
	case RSI_BLE_READ_REQ_EVENT: {
		...
		rsi_ble_gatt_read_response(remote_dev_bd_addr, 0,	app_ble_read_event.handle, 0, 	RSI_MIN(mtu_size - 2, sizeof(read_data)), read_data);
	}
		break;
