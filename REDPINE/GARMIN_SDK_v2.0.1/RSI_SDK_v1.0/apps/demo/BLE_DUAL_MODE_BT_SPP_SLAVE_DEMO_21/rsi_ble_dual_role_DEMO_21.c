/**
 * @file    rsi_ble_dual_role_DEMO_21.c
 * @version 0.1
 * @date    09 Sep 2015
 *
...
static rsi_ble_event_conn_status_t          conn_event_to_app[5];
...
static void rsi_ble_on_connect_event(rsi_ble_event_conn_status_t *resp_conn) {
  ...
  memcpy (remote_dev_bd_addr, (uint8_t *) resp_conn->dev_addr, 6);
	...
	if (strcmp((const char *) remote_dev_addr_conn1,
		...
		memcpy(remote_dev_addr_conn4, str_conn_device, 18);
		...
		memcpy(remote_dev_addr_conn5, str_conn_device, 18);
...
void rsi_ble_app_task(void) {
				...
				if (((strcmp((const char *) remote_name,
					...
					if ((conn_done == 1)) {
						status = rsi_ble_connect_with_params(remote_addr_type,
								...
								LE_SCAN_WINDOW_CONN,
								M2S1_CONNECTION_INTERVAL_MAX,
								M2S1_CONNECTION_INTERVAL_MIN,
								M2S1_CONNECTION_LATENCY,
								M2S1_SUPERVISION_TIMEOUT);
						LOG_PRINT("Conneting to device : \ndev_addr : %s\n conn_int_max: %d \nconn_int_min: %d \ntimeout: %d\n",(int8_t *) remote_dev_addr_conn, M2S1_CONNECTION_INTERVAL_MAX, M2S1_CONNECTION_INTERVAL_MIN, M2S1_SUPERVISION_TIMEOUT);
					} else if ((conn_done == 2)) {
						status = rsi_ble_connect_with_params(remote_addr_type,
								(int8_t *) remote_dev_bd_addr,
								LE_SCAN_INTERVAL_CONN,
								LE_SCAN_WINDOW_CONN,
								M2S2_CONNECTION_INTERVAL_MAX,
								M2S2_CONNECTION_INTERVAL_MIN,
								M2S2_CONNECTION_LATENCY,
								M2S2_SUPERVISION_TIMEOUT);
						LOG_PRINT("Conneting to device : \ndev_addr : %s\n conn_int_max: %d \nconn_int_min: %d \ntimeout: %d\n",(int8_t *) remote_dev_addr_conn, M2S2_CONNECTION_INTERVAL_MAX, M2S2_CONNECTION_INTERVAL_MIN, M2S2_SUPERVISION_TIMEOUT);
					} else if ((conn_done == 3)) {
						status = rsi_ble_connect_with_params(remote_addr_type,
								(int8_t *) remote_dev_bd_addr,
								LE_SCAN_INTERVAL_CONN,
								LE_SCAN_WINDOW_CONN,
								M2S3_CONNECTION_INTERVAL_MAX,
								M2S3_CONNECTION_INTERVAL_MIN,
								M2S3_CONNECTION_LATENCY,
								M2S3_SUPERVISION_TIMEOUT);
						LOG_PRINT("Conneting to device : \ndev_addr : %s\n conn_int_max: %d \nconn_int_min: %d \ntimeout: %d\n",(int8_t *) remote_dev_addr_conn, M2S3_CONNECTION_INTERVAL_MAX, M2S3_CONNECTION_INTERVAL_MIN, M2S3_SUPERVISION_TIMEOUT);
					}
					if (status != RSI_SUCCESS) {
	...
	case RSI_BLE_RECEIVE_REMOTE_FEATURES: {
...
#if 0
		if(( (strcmp((const char *) remote_dev_addr_conn4, (const char *) received_rem_devce_addr)) == 0) &&
			 ((conn_master_done & BIT(0))) && (!(conn_update_master_done & BIT(0))) ) {
				...
				status = rsi_ble_conn_params_update((int8_t *) remote_dev_addr,
		...
		else if(( (strcmp((const char *) remote_dev_addr_conn5, (const char *) received_rem_devce_addr)) == 0) &&
	...
	case RSI_BLE_READ_REQ_EVENT: {
		...
		rsi_ble_gatt_read_response(conn_event_to_app[conn_done - 1].dev_addr, 0,
				app_ble_read_event.handle, 0,
	...
	case RSI_BLE_GATT_PROFILE_RESP_EVENT: {
		...
		rsi_ble_get_char_services(conn_event_to_app[conn_done - 1].dev_addr,
...
#endif
	{
		if( (conn_update_resp.conn_interval < M2S1_CONNECTION_INTERVAL_MAX ) || (conn_update_resp.conn_latency < M2S1_CONNECTION_LATENCY ) || (conn_update_resp.timeout < M2S1_SUPERVISION_TIMEOUT ) )
		{
			conn_update_done |= BIT(0);
			//! Check for connection parameters and update the connection parameters
#if(CONNECT_OPTION == CONN_BY_NAME)
			rsi_ble_conn_params_update(conn_update_resp.dev_addr, M2S1_CONNECTION_INTERVAL_MIN, (uint16_t)M2S1_CONNECTION_INTERVAL_MAX, (uint16_t)M2S1_CONNECTION_LATENCY, (uint16_t)M2S1_SUPERVISION_TIMEOUT);
#else
			rsi_ble_conn_params_update(conn_update_resp.dev_addr, M2S1_CONNECTION_INTERVAL_MIN, (uint16_t)M2S1_CONNECTION_INTERVAL_MAX, (uint16_t)M2S1_CONNECTION_LATENCY, (uint16_t)M2S1_SUPERVISION_TIMEOUT);
#endif
			LOG_PRINT("\n Connection update initiated from RSI \n");
...
#endif
	{
		if( (conn_update_resp.conn_interval < M2S2_CONNECTION_INTERVAL_MAX ) || (conn_update_resp.conn_latency < M2S2_CONNECTION_LATENCY ) || (conn_update_resp.timeout < M2S2_SUPERVISION_TIMEOUT ) )
		{
			conn_update_done |= BIT(1);
			//! Check for connection parameters and update the connection parameters
#if(CONNECT_OPTION == CONN_BY_NAME)
			rsi_ble_conn_params_update((int8_t *)conn_update_resp.dev_addr, M2S2_CONNECTION_INTERVAL_MIN, (uint16_t)M2S2_CONNECTION_INTERVAL_MAX, (uint16_t)M2S2_CONNECTION_LATENCY, (uint16_t)M2S2_SUPERVISION_TIMEOUT);
#else
      rsi_ble_conn_params_update((int8_t *)conn_update_resp.dev_addr, M2S2_CONNECTION_INTERVAL_MIN, (uint16_t)M2S2_CONNECTION_INTERVAL_MAX, (uint16_t)M2S2_CONNECTION_LATENCY, (uint16_t)M2S2_SUPERVISION_TIMEOUT);
#endif
			LOG_PRINT("\n Connection update initiated from RSI \n");
...
#endif
	{
		if( (conn_update_resp.conn_interval < M2S3_CONNECTION_INTERVAL_MAX ) || (conn_update_resp.conn_latency < M2S3_CONNECTION_LATENCY ) || (conn_update_resp.timeout < M2S3_SUPERVISION_TIMEOUT ) )
		{
			conn_update_done |= BIT(2);
			//! Check for connection parameters and update the connection parameters
#if(CONNECT_OPTION == CONN_BY_NAME)
			rsi_ble_conn_params_update((int8_t *)conn_update_resp.dev_addr, M2S3_CONNECTION_INTERVAL_MIN, (uint16_t)M2S3_CONNECTION_INTERVAL_MAX, (uint16_t)M2S3_CONNECTION_LATENCY, (uint16_t)M2S3_SUPERVISION_TIMEOUT);
#else
			rsi_ble_conn_params_update((int8_t *)conn_update_resp.dev_addr, M2S3_CONNECTION_INTERVAL_MIN, (uint16_t)M2S3_CONNECTION_INTERVAL_MAX, (uint16_t)M2S3_CONNECTION_LATENCY, (uint16_t)M2S3_SUPERVISION_TIMEOUT);
#endif
			LOG_PRINT("\n Connection update initiated from RSI \n");
