...
int main ( int argc, char *argv[] )
{
	...
	if(get_mode == 1){
		...
		char fout2[] = "\n#define  _control_len ";
		char* fout3 = argv[4];
		char fout4[] = "\n#define  _bl_entry_control_reserved 0\n#define  _bl_entry_control_spi_32bitmode 0\n#define _bl_entry_control_release_ta_softreset 0\n#define _bl_entry_control_start_from_rom_pc 0\n#define _bl_entry_control_spi_8bitmode 0\n#define _bl_entry_control_last_entry 1\n#define _bl_entry_dst_addr 0" ;
		char* File_create_with;
		File_create_with = malloc(strlen(fout)+strlen(fout1)+strlen(fout2)+strlen(fout3)+strlen(fout4)); /* make space for the new string (should check the return value ...) */
