...
//! Security type
#define SECURITY_TYPE     RSI_WPS_PIN
...
if(SECURITY_TYPE == RSI_USE_GENERATED_WPSPIN) 
{
  //!Generate WPS pin
  status = rsi_wlan_wps_generate_pin(wps_pin,10);
  if(status != RSI_SUCCESS)
  {
    return status;
  }
  ...
  //! Connect to an Access point WPS pin method
  //! Need not send AP ssid
  if(SECURITY_TYPE == RSI_USE_GENERATED_WPSPIN)
  {
    status = rsi_wlan_connect(SSID, SECURITY_TYPE, wps_pin);
  }
  else
  {
