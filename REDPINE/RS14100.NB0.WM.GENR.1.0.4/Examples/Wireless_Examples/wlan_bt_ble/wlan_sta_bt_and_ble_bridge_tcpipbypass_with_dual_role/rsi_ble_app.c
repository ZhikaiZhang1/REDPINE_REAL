...
#define IN_SCANNING_STATE_ALREADY      0x4E0C
...
uint8_t conn_str_bd_address[18];
uint8_t disc_str_bd_address[18];
...
void rsi_ble_simple_central_on_adv_report_event(rsi_ble_event_adv_report_t *adv_report)
{
  ...
  LOG_PRINT("\n DEV_ADDR: before setting adv event%s\r\n", remote_dev_addr);
...
void rsi_ble_on_enhance_conn_status_event(rsi_ble_event_enhance_conn_status_t *resp_enh_conn)
{
  ...
  rsi_6byte_dev_address_to_ascii ((int8_t *)conn_str_bd_address, (uint8_t *)resp_enh_conn->dev_addr);	
...
static void rsi_ble_on_disconnect_event(rsi_ble_event_disconnect_t *resp_disconnect, uint16_t reason)
{
  ...
  rsi_6byte_dev_address_to_ascii ((int8_t *)disc_str_bd_address, (uint8_t *)resp_disconnect->dev_addr);	
...
#if RSI_DUAL_ROLE
        if(!strcmp((char *)conn_str_bd_address,RSI_BLE_DEV_ADDR)) {
...
#if RSI_DUAL_ROLE
        rsi_ble_connected = 0;
        if (!strcmp((char *)disc_str_bd_address,RSI_BLE_DEV_ADDR))
        {
...
scan:
          ...
          rsi_app_no_of_adv_reports_rcvd = 0;
          status = rsi_ble_start_scanning();
          if((status != RSI_SUCCESS) && (status == IN_SCANNING_STATE_ALREADY))
          {
            goto scan;
          }
        } else {
    ...
    case RSI_BLE_GATT_WRITE_EVENT:
      {
        ...
        for (ix = 0; ix < rsi_ble_app_data_len; ix += RSI_BLE_MAX_DATA_LEN) {
          //! set the local attribute value.
          rsi_ble_set_local_att_value (rsi_ble_att2_val_hndl, RSI_BLE_MAX_DATA_LEN, (rsi_ble_app_data + ix));
        }
      }
      break;
  }
