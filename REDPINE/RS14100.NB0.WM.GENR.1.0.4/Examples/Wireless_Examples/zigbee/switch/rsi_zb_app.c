...
#define ZIGB_GLOBAL_BUFF_LEN                15000

#define RSI_DELAY                           0xFFFF
#else
...
int32_t rsi_zigb_app()
{
  ...
  status = rsi_zigb_initiate_scan(g_MAC_ACTIVE_SCAN_TYPE_c,g_CHANNEL_MASK_c, g_SCAN_DURATION_c);
  if(status != RSI_SUCCESS)
  {
	  return status;
  }
  ...
  status =rsi_zigb_join_network(device_type, rsi_zigb_app_info.nwkinfo.channel, 0x0f, rsi_zigb_app_info.nwkinfo.extendedPanId);
  if(status != RSI_SUCCESS)
  {
	  return status;
  }
  ...
  status = rsi_zigb_set_simple_descriptor(ONOFF_SWITCH_END_POINT, rsi_zigb_app_info.DeviceSimpleDesc);
  if(status != RSI_SUCCESS)
  {
	  return status;
  }
