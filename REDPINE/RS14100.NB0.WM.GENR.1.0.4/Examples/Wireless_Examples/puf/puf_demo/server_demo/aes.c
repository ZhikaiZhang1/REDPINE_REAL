...
static int AesSetKeyLocal(Aes* aes, const byte* userKey, word32 keylen,	const byte* iv, int dir)
{
...
void encrypt_data_with_key_16(unsigned char *in, unsigned int length, unsigned char *out, unsigned char *key)  
{
	Aes aes;
	int i = 0;

	AesSetKeyLocal(&aes, key, 16, NULL, AES_ENCRYPTION);

	for(i = 0; i < length; i = i + AES_BLOCK_SIZE) {
		AesEncrypt(&aes, &in[i], &out[i]);
	}
	return;
}
