  ...
  switch(rsi_wlan_app_cb.state)
  {
    ...
    case RSI_WLAN_AP_UP_STATE:
      { 
        ...
        //! Create socket
        server_socket = rsi_socket_async(AF_INET, SOCK_STREAM, 0, rsi_wlan_async_data_recv);
        if(server_socket < 0)
        {
    ...
    case RSI_WLAN_SOCKET_CONNECTED_STATE:
      {
        ...
        {
          //! send data on socket
          status = rsi_send(new_socket, (const int8_t *)rsi_wlan_app_cb.buffer, rsi_wlan_app_cb.length, 0);
          if(status < 0)
          {
            status = rsi_wlan_get_status();
            break;
