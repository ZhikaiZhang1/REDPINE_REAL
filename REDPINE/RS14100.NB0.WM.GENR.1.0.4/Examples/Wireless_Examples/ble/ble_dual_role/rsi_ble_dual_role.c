...
#define RSI_BLE_DEV_M1_ADDR      "00:1A:7D:34:54:65"
#define RSI_BLE_DEV_M2_ADDR      "00:1A:7D:DA:71:63"
...
void rsi_ble_on_enhance_conn_status_event(rsi_ble_event_enhance_conn_status_t *resp_enh_conn)
{
  ...
  else if (!strcmp((char *)str_conn_device, RSI_BLE_DEV_M1_ADDR))
  {
    conn_event_to_app[8].dev_addr_type = resp_enh_conn->dev_addr_type;
    memcpy (conn_event_to_app[8].dev_addr, resp_enh_conn->dev_addr, RSI_DEV_ADDR_LEN);
    conn_event_to_app[8].status = resp_enh_conn->status;
    num_of_conn_masters ++;
  }
  else if (!strcmp((char *)str_conn_device, RSI_BLE_DEV_M2_ADDR))
  {
    conn_event_to_app[9].dev_addr_type = resp_enh_conn->dev_addr_type;
    memcpy (conn_event_to_app[9].dev_addr, resp_enh_conn->dev_addr, RSI_DEV_ADDR_LEN);
    conn_event_to_app[9].status = resp_enh_conn->status;
    num_of_conn_masters ++;
  }
...
static void rsi_ble_on_connect_event(rsi_ble_event_conn_status_t *resp_conn)
{
  ...
  {
    memcpy(&conn_event_to_app[8], resp_conn, sizeof(rsi_ble_event_conn_status_t));
    num_of_conn_masters ++;
  }
  else if (!strcmp((char *)str_conn_device, RSI_BLE_DEV_M2_ADDR))
  {
    memcpy(&conn_event_to_app[9], resp_conn, sizeof(rsi_ble_event_conn_status_t));
    num_of_conn_masters ++;
  }
...
static void rsi_ble_on_disconnect_event(rsi_ble_event_disconnect_t *resp_disconnect, uint16_t reason)
{
  ...
  else if (!strcmp((char *)str_disconn_device, RSI_BLE_DEV_M1_ADDR))
  {
    memset(&conn_event_to_app[8], 0, sizeof(rsi_ble_event_conn_status_t));
    num_of_conn_masters --;
  }
  else if (!strcmp((char *)str_disconn_device, RSI_BLE_DEV_M2_ADDR))
  {
    memset(&conn_event_to_app[9], 0, sizeof(rsi_ble_event_conn_status_t));
    num_of_conn_masters --;
  }
          ...
          if(conn_req_pending == 0)
          {
            if(num_of_conn_slaves < MAX_NUM_OF_SLAVES)
            {
                  ...
                  (!strcmp((char *)conn_dev_addr, RSI_BLE_DEV_8_ADDR)))
              {
                ...
                LOG_PRINT("\n Connect command \n");
                status = rsi_ble_connect(RSI_BLE_DEV_ADDR_TYPE, (int8_t *)conn_dev_addr);
                if(status != RSI_SUCCESS)
                {
                  return status;
                }
