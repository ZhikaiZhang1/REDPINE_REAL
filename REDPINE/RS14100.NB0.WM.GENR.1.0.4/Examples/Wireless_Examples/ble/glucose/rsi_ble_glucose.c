 ...
 *
 *  @brief : This file contains application for BLE Glucose service
 *
...
static void rsi_ble_on_smp_request (rsi_bt_event_smp_req_t  *remote_dev_address)
{
  ...
  LOG_PRINT ("smp_req - str_remote_address : %s", str_remote_address);
...
{
  rsi_6byte_dev_address_to_ascii (str_remote_address, remote_dev_address->dev_addr);
  LOG_PRINT ("smp_resp - str_remote_address : %s", str_remote_address);
...
{
  rsi_6byte_dev_address_to_ascii (str_remote_address, remote_dev_address->dev_addr);
  LOG_PRINT ("smp_passkey - str_remote_address : %s", str_remote_address);
...
{
  rsi_6byte_dev_address_to_ascii (str_remote_address, remote_dev_address->dev_addr);
  LOG_PRINT ("smp_failed status: 0x%x, str_remote_address: %s", status, str_remote_address);
...
static uint32_t rsi_ble_add_glucose_serv (void)
{
  ...
  rsi_ble_add_service (new_uuid, 12, 120, &new_serv_resp);
...
int32_t rsi_ble_glucose_gatt_server(void)
{
  ...
  //! Set local name
  rsi_bt_set_local_name(RSI_BLE_APP_GLUCOSE);
  ...
  adv[4] = 9;
  strcpy (&adv[5], RSI_BLE_APP_GLUCOSE);
...
get_prof:          
          ...
          status = rsi_ble_get_profile_async (remote_dev_addr, service_uuid, &ble_servs);
          if(status != 0)
          {
            goto get_prof;
          }
...
scan:
      ...
      case RSI_BLE_SMP_PASSKEY_DISPLAY_EVENT:
      {
        ...
        //rsi_ble_smp_passkey (str_remote_address, RSI_BLE_SMP_PASSKEY);        
      }
      break;
