 ...
 *
 *  @brief : This file contains example application for BLE Battery Service
 *
 *  @section Description  This application connects as a peripheral and loops back the data
 *  received from the GATT client in GATT server case.
 *  When our module act as GATT client it will connect to the remote device based on BDaddress given in the application
 *  and get services from the remote GATT server.
 *
 */
      ...
      case RSI_BLE_CONN_EVENT:
        {
          ...
          rsi_ble_get_profile_async (remote_dev_addr, query_serv, &get_profile);
#endif
        }
        break;
      ...
      case RSI_BLE_GATT_PROFILE:
        {
          ...
          rsi_ble_get_char_services_async (remote_dev_addr, *((uint16_t *)get_profile.start_handle), *((uint16_t *)get_profile.end_handle), &get_char_services);
        }
        break;
      ...
      case RSI_BLE_GATT_DESC_SERVICES:
        {
          ...
          for(ix=0;ix<get_desc_services.num_of_att;ix++){
            ...
            if(get_desc_services.att_desc[ix].att_type_uuid.val.val16 == RSI_BLE_CLIENT_CHAR_UUID)
            {
              ...
              rsi_ble_set_att_value_async(remote_dev_addr,*((uint16_t *)get_desc_services.att_desc[ix].handle),2,data); 
            }
          }	
        }
        break;
