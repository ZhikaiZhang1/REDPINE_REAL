 ...
 *
 *  @brief : This file contains example application for BLE Central mode with white list support.
 *
 *  @section Description  This application adds devices in to white list.
 *
 */
...
int32_t rsi_ble_whitelist(void)
{
  ...
  status = rsi_ble_addto_whitelist(RSI_BLE_WHITELIST_DEV_ADDR2,RSI_BLE_WHITELIST_DEV_ADDR2_TYPE);
  if(status != RSI_SUCCESS)
  {
    return status;
  }
