  ...
  //! waiting for events from controller.
	while (1) {
...
#ifdef RSI_BLE_PING
        //! get ping time out
        rsi_ble_get_le_ping_timeout(str_remote_address,&timeout_resp);

        //! set le ping time out
        rsi_ble_set_le_ping_timeout (str_remote_address,RSI_BLE_PING_TIME_OUT);
      ...
      case RSI_BLE_SC_PASSKEY_EVENT:
      {
        ...
        rsi_ble_smp_passkey (str_remote_address, numeric_value);
      }
      break;
      ...
      case RSI_BLE_LTK_REQ_EVENT:
      {
        ...
        if (encrypt_keys.localediv == ble_ltk_req.localediv) {
          rsi_ble_ltk_req_reply (str_remote_address, 1, encrypt_keys.localltk);
        } else {
          rsi_ble_ltk_req_reply (str_remote_address, 0, NULL);
        }
      }
      break;
