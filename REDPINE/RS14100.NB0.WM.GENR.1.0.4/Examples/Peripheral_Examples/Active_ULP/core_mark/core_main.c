...
void *iterate(void *pres) {
	...
	while(1) {
		if(yyyy == 1) {
			RSI_EGPIO_SetPin(EGPIO1 ,EGPIO_PORT0 , GPIO3 ,1);
			yyyy = 0;
		} else {
			RSI_EGPIO_SetPin(EGPIO1 ,EGPIO_PORT0 , GPIO3 ,0);
			yyyy = 1;
		}
...
*/
volatile uint32_t iterations_per_second;
#if MAIN_HAS_NOARGC
...
#error "Please define a way to initialize a memory block."
  ...
  iterations_per_second = default_num_contexts*results[0].iterations/time_in_secs(total_time);
