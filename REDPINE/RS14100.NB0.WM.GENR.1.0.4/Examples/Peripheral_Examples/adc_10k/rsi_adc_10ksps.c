 ...
 *
 * @brief This files contains example code of ADC functionality for 10ksps 
 *        as sampling rate with sleep wakeup.
 * @section Description
 * This files contains example code of ADC 10ksps sampling rate. 
 *
 *
 */
/*Includes*/
...
/**
 * @brief  This function configure ADC sampling rate as 10ksps.
 * @param  None
 * @retval None
 */
void adc_clk_config()
{
#ifndef RO_1MHZ_CLOCK
	 ...
	 RSI_ADC_ChannelSamplingRate(AUX_ADC_DAC_COMP,CHANNEL_NO,1,4); // channel , ch offset , ch freq val //8KSPS
#else
	 RSI_ADC_ChannelSamplingRate(AUX_ADC_DAC_COMP,CHANNEL_NO,1,50); // channel , ch offset , ch freq val //10KSPS
#endif
}
...
void ADC_10k(void)
{
  ...
  RSI_ADC_Config(AUX_ADC_DAC_COMP,1,0,FIFO_THR,1);
	...
	RSI_ADC_PingPongMemoryAdrConfig(AUX_ADC_DAC_COMP,CHANNEL_NO,PING_ADDRESS,PONG_ADDRESS,PING_LEN,PONG_LEN,1,1);
  ...
  /* Configure the ADC sampling rate as 10ksps or 8ksps */ 
	adc_clk_config();
