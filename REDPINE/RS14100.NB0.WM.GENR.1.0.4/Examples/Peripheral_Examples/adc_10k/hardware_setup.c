...
void hardware_setup()
{
  ...
  RSI_ULPSS_PeripheralDisable(ULPCLK,ULP_VAD_CLK);
	...
	/* Change ULPSS-REF and M4SS-REF and NPSS-HF_FSM clocks to 20MHz RC to be used in PS2 state */
  ...
  /* Power-Down Unused ULPSS Domains */
	...
	RSI_PS_SetRamRetention(	M4ULP_RAM16K_RETENTION_MODE_EN | M4ULP_RAM_RETENTION_MODE_EN 
	                        | ULPSS_RAM_RETENTION_MODE_EN ); /* Enable SRAM Retention during Sleep */	
}
