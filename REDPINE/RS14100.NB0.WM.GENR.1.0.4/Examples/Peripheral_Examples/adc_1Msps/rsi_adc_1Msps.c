 ...
 *
 * @brief This files contains example code of ADC functionality for 1Msps 
 *        as sampling rate with sleep wakeup. 
 * @section Description
 * This files contains example code of ADC 1Msps sampling rate.
 *
 *
 */
/*Includes*/
...
void ADC_1000(void)
{
	...
	RSI_ADC_ClkDivfactor(AUX_ADC_DAC_COMP,0,5);	
  ...
  RSI_ADC_Config(AUX_ADC_DAC_COMP,1,0,FIFO_THR,1);	
	...
	RSI_ADC_PingPongMemoryAdrConfig(AUX_ADC_DAC_COMP, CHANNEL_NO, PING_ADDRESS, 
	                                PONG_ADDRESS, PING_LEN, PONG_LEN, 1, 1);
  ...
  adc_input_pin_mux(1);
	...
	/* Configure the ADC sampling rate as 1msps */ 
  RSI_ADC_ChannelSamplingRate(AUX_ADC_DAC_COMP,CHANNEL_NO,1,2); // channel , ch offset , ch freq val
	
  RSI_ADC_NoiseAvgMode(AUX_ADC_DAC_COMP,1);
