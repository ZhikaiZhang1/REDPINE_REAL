...
void rsi_tx_event_handler(void)
{
...
#endif
  {
    ...
    if (frame_type == RSI_COMMON_REQ_PWRMODE)
    {
...
#if (RSI_HAND_SHAKE_TYPE != GPIO_BASED)
  if ((frame_type == RSI_COMMON_RSP_ULP_NO_RAM_RETENTION) ||(frame_type == RSI_RSP_SLP) || (frame_type == RSI_RSP_WKP))
  {
...
#endif    
#ifdef SPI_INTERFACE
	   if(!rsi_hal_get_gpio(RSI_HAL_INTR_ST_GPIO))
