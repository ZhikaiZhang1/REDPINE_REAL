...
{
  rsi_bt_avrcp_ele_att_resp_t  song_att = {{0}};
  uint8_t  ix;
  
  
  rsi_ascii_dev_address_to_6bytes_rev(song_att.dev_addr, (int8_t *)remote_dev_addr);
  song_att.song_atts.nbr_atts = nbr_atts;
  for (ix = 0; ix < nbr_atts; ix++) {
    song_att.song_atts.list[ix].id = p_att_list[ix].id;
    memcpy (song_att.song_atts.list[ix].att_text, p_att_list[ix].att_text, strlen (p_att_list[ix].att_text));
  }
  
  return rsi_bt_driver_send_cmd(RSI_BT_REQ_AVRCP_GET_ELEMENT_ATT_RESP, &song_att, NULL);
}
...
int32_t rsi_bt_avrcp_att_text_resp (uint8_t *remote_dev_addr, uint8_t nbr_atts, att_text_t *p_att_text)
{
  ...
  for (ix = 0; ix < nbr_atts; ix++)
  {
    ...
    memcpy (att_text.atts_text.list[ix].att_text, p_att_text[ix].att_text, strlen (p_att_text[ix].att_text));
  }
...
int32_t rsi_bt_avrcp_att_vals_text_resp (uint8_t *remote_dev_addr, uint8_t nbr_vals, att_text_t *p_vals_text)
{
  ...
  for (ix = 0; ix < nbr_vals; ix++)
  {
    ...
    memcpy (vals_text.atts_text.list[ix].att_text, p_vals_text[ix].att_text, strlen (p_vals_text[ix].att_text));
  }
