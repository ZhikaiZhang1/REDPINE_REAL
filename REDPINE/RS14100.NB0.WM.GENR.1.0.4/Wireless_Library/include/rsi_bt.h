...
typedef enum rsi_bt_cmd_request_e
{
  ...
  RSI_BT_REQ_A2DP_BURST_MODE                     = 0x00CF,
...
typedef enum rsi_bt_event_e
{
 ...
 /*AVRCP profile Event ID's*/
  ...
  RSI_BT_EVT_AVRCP_NOTIFY                        = 0x1310,
...
//! A2DP song end indication command
typedef struct rsi_bt_req_a2dp_burst_mode_s
{
  uint8_t  dev_addr[RSI_DEV_ADDR_LEN];
  uint8_t  enable;
} rsi_bt_req_a2dp_burst_mode_t;
...
typedef struct rsi_bt_avrcp_ele_att_resp_s
{
  uint8_t  dev_addr[RSI_DEV_ADDR_LEN];
  uint8_t  reserved[2];
  player_att_text_t song_atts;
} rsi_bt_avrcp_ele_att_resp_t;
