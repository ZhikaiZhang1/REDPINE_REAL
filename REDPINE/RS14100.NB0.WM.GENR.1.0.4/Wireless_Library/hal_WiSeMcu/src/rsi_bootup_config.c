...
int16_t rsi_select_option(uint8_t cmd)
{
  ...
  if((cmd != RSI_LOAD_IMAGE_I_FW) && (cmd != RSI_LOAD_IMAGE_I_ACTIVE_LOW_FW) && (cmd !=  RSI_JUMP_TO_PC))
  {
    RSI_RESET_LOOP_COUNTER(loop_counter);
  ...
  else if((cmd == RSI_LOAD_IMAGE_I_FW) || (cmd == RSI_LOAD_IMAGE_I_ACTIVE_LOW_FW) || (cmd ==  RSI_JUMP_TO_PC))
	{
		retval = rsi_boot_insn(REG_READ, &read_value);
    if(retval < 0)
    {
