 ...
 * @return      execution status.
 */
error_t RSI_IPMU_RetnLdoLpmode(void)
{
	return RSI_IPMU_ProgramConfigData(retn_ldo_lpmode);
} 

/**
 * @fn         error_t RSI_IPMU_RetnLdoHpmode(void)
 * @brief	    This API is used to program enabling the RETN_LDO HP MODE .
 * @return      execution status.
 */
error_t RSI_IPMU_RetnLdoHpmode(void)
{
	return RSI_IPMU_ProgramConfigData(retn_ldo_hpmode);
} 
/**
 ...
 * @brief       This API is used to trim the ROMhz clock to required frequency.
 * @return      none 
 */
void RSI_IPMU_20M_ROClktrim(uint8_t clkfreq)
{
	...
	if(clkfreq > 10)
	{
		...
		ULP_SPI_MEM_MAP(ULPCLKS_HFRO_CLK_REG_ADDR) |=  BIT(13) ;  
	}
	else
	{
	...
	ULP_SPI_MEM_MAP(ULPCLKS_HFRO_CLK_REG_ADDR) |= BIT(21) | BIT(12);  	
	...
	ULP_SPI_MEM_MAP(ULPCLKS_HFRO_CLK_REG_ADDR) &= ~(MASK_TRIM_VALUE_WRITE_BITS);
	/* Programming the calibrated trim to SPI register. */
	ULP_SPI_MEM_MAP(ULPCLKS_HFRO_CLK_REG_ADDR) |= (ro50m_trim << 14) ;
	...
	ULP_SPI_MEM_MAP(ULPCLKS_HFRO_CLK_REG_ADDR) &= ~BIT(21);  
}
