...
{	
  ...
  RSI_UDMA_DATACONTEXT_T *pDrv = (RSI_UDMA_DATACONTEXT_T *) pHandle;
	...
	if((channelTableIndex & 0xffff) < 64)
	{
      ...
      {			
        length = ( vsUdmaChaConfigData.totalNumOfDMATrans << srcInc );
        pSrcAddr = (void *)((uint32_t)pSrcAddr + length );
      }
      ...
      {   
        ...
        {	
          length = vsUdmaChaConfigData.totalNumOfDMATrans << dstInc;
          pDstAddr = (void *)((uint32_t)pDstAddr + length);
        }
      }
