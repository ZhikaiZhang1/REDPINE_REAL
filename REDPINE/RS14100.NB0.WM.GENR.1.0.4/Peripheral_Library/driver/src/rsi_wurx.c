...
{  
	 volatile  uint32_t cntr=0,lco_coarse,lco_fine;	
   if(clk_enable)
   {		
...
void RSI_IPMU_DCCalib()
{
  ...
  uint32_t cntr;
	WURX_COMP_OFFSET_CALIB_REG |= (( VAL1 << POS10) | 
	                                ...
	                                ( VAL2 << POS1));              // 5% periodic auto calibration    
...
void RSI_WURX_ReadPattern1Odd(uint32_t *tail_data)
{
  ...
  tail_data[VAL0]=WURX_ODD_PATTERN1_REG_LSB;            // 22 bit for LSB
	read_tail_data=WURX_ODD_PATTERN1_REG_MID;
	read_tail_data=(read_tail_data<<POS22);      
	tail_data[VAL0]=tail_data[VAL0] | read_tail_data;     // 10 bit read
	read_tail_data=0;
	read_tail_data=WURX_ODD_PATTERN1_REG_MID;
	read_tail_data=(read_tail_data>>POS10);
	tail_data[VAL1]=read_tail_data;                       // 12 bit read	
	read_tail_data=0;
  read_tail_data=WURX_ODD_PATTERN1_REG_MSB;
	read_tail_data=read_tail_data<<POS12;
  tail_data[VAL1]=tail_data[VAL1]|read_tail_data;       // 20 bit read
}
...
{
	uint32_t read_tail_data=0;		
  tail_data[VAL2]=WURX_EVEN_PATTERN1_REG_LSB;          // 22 bit for LSB
	read_tail_data=WURX_EVEN_PATTERN1_REG_MID;
	read_tail_data=(read_tail_data<<POS22);      
	tail_data[VAL2]=tail_data[VAL2] | read_tail_data;    // 10 bit read
	read_tail_data=0;
	read_tail_data=WURX_EVEN_PATTERN1_REG_MID;
	read_tail_data=(read_tail_data>>POS10);
	tail_data[VAL3]=read_tail_data;                      // 12 bit read	
	read_tail_data=0;
  read_tail_data=WURX_EVEN_PATTERN1_REG_MSB;
	read_tail_data=read_tail_data<<POS12;
	tail_data[VAL3]=tail_data[VAL3]|read_tail_data;      // 20 bit read	
}
...
{
  uint32_t read_tail_data=0;	 
  tail_data[VAL4]=WURX_ODD_PATTERN2_REG_LSB;            // 22 bit for LSB
	read_tail_data=WURX_ODD_PATTERN2_REG_MID;
	read_tail_data=(read_tail_data<<POS22);      
	tail_data[VAL4]=tail_data[VAL4] | read_tail_data;     // 10 bit read
	read_tail_data=0;
	read_tail_data=WURX_ODD_PATTERN2_REG_MID;
	read_tail_data=(read_tail_data>>POS10);
	tail_data[VAL5]=read_tail_data;                       // 12 bit read	
	read_tail_data=0;
  read_tail_data=WURX_ODD_PATTERN2_REG_MSB;
	read_tail_data=read_tail_data<<POS12;
	tail_data[VAL5]=tail_data[VAL5]|read_tail_data;       // 20 bit read	
}
...
{
  uint32_t read_tail_data=0;	 
  tail_data[VAL6]=WURX_ODD_PATTERN2_REG_LSB;            // 22 bit for LSB
	read_tail_data=WURX_ODD_PATTERN2_REG_MID;
	read_tail_data=(read_tail_data<<POS22);      
	tail_data[VAL6]=tail_data[VAL6] | read_tail_data;    // 10 bit read
	read_tail_data=0;
	read_tail_data=WURX_ODD_PATTERN2_REG_MID;
	read_tail_data=(read_tail_data>>POS10);
	tail_data[VAL7]=read_tail_data;                      // 12 bit read	
	read_tail_data=0;
  read_tail_data=WURX_ODD_PATTERN2_REG_MSB;
	read_tail_data=read_tail_data<<POS12;
	tail_data[VAL7]=tail_data[VAL7]|read_tail_data;      // 20 bit read	
}
