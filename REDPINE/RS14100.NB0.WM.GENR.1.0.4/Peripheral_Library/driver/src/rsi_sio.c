...
volatile int u32TxIdx=0 , u32rxIdx =0 ;
		...
		if((pstcSio->SIO_SWAP_INTR_STATUS_REG) & (1 << gstcSioCb.uart_sio.u8UartRxCh))
		{
			if(gstcSioCb.uart_sio.u16UartRxDone < gstcSioCb.uart_sio.u16UartRxCnt)
			{
				...
				if ( gstcSioCb.uart_sio.pstcSioUart->u8Bitlen == 8) {
					...
					((uint16_t *) gstcSioCb.uart_sio.pvUartRx)[gstcSioCb.uart_sio.u16UartRxDone] = (u32Data & 0XFF) ;
				}
				/*Check the buffer lengths */
 ...
 * @return       error codes
 */
error_t RSI_SIO_I2cWrite(volatile SIO_Type *pstcSio , stc_sio_i2c_config_t *pstcConfig ,uint8_t u8SlaveAddr , uint8_t *u8Data , uint16_t  u16Len)
{
 ...
 * @return       error codes
 */
error_t RSI_SIO_I2cRead(volatile SIO_Type *pstcSio , stc_sio_i2c_config_t *pstcConfig ,uint8_t u8SlaveAddr , uint8_t *u8Data , uint16_t u16Len)
{
	
	uint32_t u32Enable = 0 , u32Sdat =0 , u32Slav =0;
	uint32_t u32RecvDat =0 ;
	int cnt;
	...
	//pstcSio->SHIFT_COUNT_PRELOAD_REG[6] =  (u32I2cClok << 0 );
 ...
 * @return       return \ref RSI_OK on success and on failure return error code. 
 */
error_t RSI_SIO_I2cTransfer(volatile SIO_Type *pstcSio , stc_sio_i2c_config_t *pstcConfig ,uint8_t u8SlaveAddr , uint8_t *u8PtrTxDat , uint16_t  u16TxLen ,uint8_t *u8PtrRxDat , uint16_t  u16RxLen  )
{
	uint32_t u32Enable = 0;
	uint32_t u32RecvDat =0  , u32I2cClok =0 ;
	uint32_t u32Sdat =0  , u32Slav =0;
